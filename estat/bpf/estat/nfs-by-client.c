...
#define IO_SUBTYPE_LEN  9
#define AXIS_LENGTH  (CLIENT_LEN)
...
static int aggregate_data(nfs_data_t *data, u64 ts, u32 type, char *verstr)
{
    ...
    if (type == IO_TYPE_WRITE) {
        ...
        name[8] = '/';
        if (data->sync == 1) {
          __builtin_memcpy(&name[9], "sync", IO_SUBTYPE_LEN);
        } else {
          __builtin_memcpy(&name[9], "async", IO_SUBTYPE_LEN);
        }
    } else if (type == IO_TYPE_READ) {
        __builtin_memcpy(&name[3], READ_STR, OP_NAME_LEN + 1);
        name[7] = '/';
        if (data->cached) {
          __builtin_memcpy(&name[8], "cached", IO_SUBTYPE_LEN);
        } else {
          __builtin_memcpy(&name[8], "uncached", IO_SUBTYPE_LEN);
        }
    } else {
        return 0;
    }
    ...
    if (data->client[0] == '*') {
	    __builtin_memcpy(&axis, &(data->client[CLIENT_PREFIX_LEN]), AXIS_LENGTH);
    } else if (data->client[0] == ',') {
	    __builtin_memcpy(&axis, &(data->client[CLIENT_PREFIX_LEN - 1]), AXIS_LENGTH);
    } else {
	    __builtin_memcpy(&axis, data->client, AXIS_LENGTH);
    }
...
static int nfsd3_aggregate_data(u64 ts, u32 type)
{
    u32 pid = bpf_get_current_pid_tgid();
    nfs_data_t *data = nfs_base_data.lookup(&pid);
