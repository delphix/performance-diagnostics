...
def aggr_thread(tbd):
    ...
    while True:
        ...
        print('{:<25}'.format(my_time) +\
            '{:<15}'.format('Aggregate') +\
            '{:<35}'.format('') +\
            '{:<10}'.format('  ' + str(throughput)) +\
            '{:<10}'.format(str(cthroughput)) )
...
def dsp_server(nexus,aggr=False):
	...
	# XXX - add support for Connector and Grid
        ...
        while True:
                # python2 doesn't support text option to popen but python3 needs text to return output as a str as opposed to bytes
                try:
                    stri = subprocess.Popen([jmxtool, 'dsp', stat_name, nexus], stdout=subprocess.PIPE,text=True).communicate()[0].splitlines()
                except:
                    stri = subprocess.Popen([jmxtool, 'dsp', stat_name, nexus], stdout=subprocess.PIPE).communicate()[0].splitlines()
                for line in stri:
                ...
                cmds = int(dict['server.sum.totalCompleted'][0]) - int(prev_dict['server.sum.totalCompleted'][0])

                if cmds > active_cmd_threshold:
                        my_time=time.strftime('%Y-%m-%d %H:%M:%S %Z',time.localtime())
                        disp_lock.acquire()

                        complete_time = int((int(dict['server.sum.completeTime'][0]) - int(prev_dict['server.sum.completeTime'][0])) / cmds / NS_TO_US)
                        dispatch_time = int((int(dict['server.sum.dispatchTime'][0]) - int(prev_dict['server.sum.dispatchTime'][0])) / cmds / NS_TO_US)
                        execute_time = int((int(dict['server.sum.executeTime'][0]) - int(prev_dict['server.sum.executeTime'][0])) / cmds / NS_TO_US)
                        pending_time = int((int(dict['server.sum.pendingTime'][0]) - int(prev_dict['server.sum.pendingTime'][0])) / cmds / NS_TO_US)
                        process_time = int((int(dict['server.sum.processTime'][0]) - int(prev_dict['server.sum.processTime'][0])) / cmds / NS_TO_US)
                        service_time = int((int(dict['server.sum.serviceTime'][0]) - int(prev_dict['server.sum.serviceTime'][0])) / cmds / NS_TO_US)
                        dsp_avg_queue = int((int(dict['server.sum.totalActive'][0]) - int(prev_dict['server.sum.totalActive'][0])) / cmds)
                        ...
                        if aggr:
                            print('{:<25}'.format(my_time) +\
                                    '{:<15}'.format(tag_uuid) +\
                                    '{:>60}'.format('  |  ') +\
                                    '{:>10}'.format(dsp_avg_queue) +\
                                    '{:>10}'.format(iops) +\
                                    '{:>15}'.format(execute_time) +\
                                    '{:>15}'.format(pending_time) +\
                                    '{:>15}'.format(service_time))
                        else:
                            if (nexus.count('Export')):
                                print( '{0:>65} :: {1}'.format(my_time,'DSP Server(target host side)'))
                            else:
                                print( '{0:>65} :: {1}'.format(my_time,'DSP Server'))
                            print('{0:>65} : {1} us'.format('Average complete time', complete_time))
                            print('{0:>65} : {1} us'.format('Average dispatch time', dispatch_time))
                            print('{0:>65} : {1} us'.format('Average execute time', execute_time))
                            print('{0:>65} : {1} us'.format('Average pending time', pending_time))
                            print('{0:>65} : {1} us'.format('Average process time', process_time))
                            print('{0:>65} : {1} us'.format('Average service time', service_time))
                            print('{0:>65} : {1}'.format('Average DSP queue', dsp_avg_queue))
...
#
def dsp_client(nexus,aggr=False):
        global NS_TO_US
        global aggr_data
        dict = { }
        prev_dict = { }
	...
	# XXX - add support for Connector and Grid
        ...
        while True:
                ...
                if cmds > active_cmd_threshold:
                        ...
                        complete_time = int((int(dict['client.sum.completeTime'][0]) - int(prev_dict['client.sum.completeTime'][0])) / cmds / NS_TO_US)
                        dispatch_time = int((int(dict['client.sum.dispatchTime'][0]) - int(prev_dict['client.sum.dispatchTime'][0])) / cmds / NS_TO_US)
                        execute_time = int((int(dict['client.sum.executeTime'][0]) - int(prev_dict['client.sum.executeTime'][0])) / cmds / NS_TO_US)
                        network_time = int((int(dict['client.sum.networkTime'][0]) - int(prev_dict['client.sum.networkTime'][0])) / cmds / NS_TO_US)
                        pending_time = int((int(dict['client.sum.pendingTime'][0]) - int(prev_dict['client.sum.pendingTime'][0])) / cmds / NS_TO_US)
                        dsp_avg_queue = int((int(dict['client.sum.totalActive'][0]) - int(prev_dict['client.sum.totalActive'][0])) / cmds)
                        ...
                        if aggr:
                            ...
                            print('{0:>25} : {1} us'.format('Average complete time', complete_time))
                            print('{0:>25} : {1} us'.format('Average dispatch time', dispatch_time))
                            print('{0:>25} : {1} us'.format('Average execute time', execute_time))
                            print('{0:>25} : {1} us'.format('Average network time', network_time))
                            print('{0:>25} : {1} us'.format('Average pending time', pending_time))
                    ...
                    print(nexus + ' : Idle Nexus : Active Command Threshold is ' + str(active_cmd_threshold) + ' and only ' +\
                                str(cmds) + ' command(s) completed in the last interval.')
...
def print_aggr_header():
    print('{:^95}'.format('Client') +\
        '{:^100}'.format('Server') )

    print('{:>95}'.format('| Throughput KB/sec|') +\
        '{:>5}'.format('  |  '))
    print('{:<25}'.format('Date/Time') +\
        '{:<15}'.format('TAG') +\
        '{:<10}'.format('Queue') +\
        '{:<10}'.format('IOPS') +\
        '{:<15}'.format('Network (us)') +\
        '{:<10}'.format('| Raw') +\
        '{:<10}'.format('Compress |')  +\
        '{:>5}'.format('  |  ') +\
        '{:>10}'.format('Queue') +\
        '{:>10}'.format('IOPS') +\
        '{:>15}'.format('Execute (us)') +\
        '{:>15}'.format('Pending (us)') +\
        '{:>15}'.format('Service (us)') )
...
parser.add_argument('--aggr', action='store_true',help='aggregate display')
